...
export default function Shopping() {
  const beaches = places.filter((place) => place.category === "shopping");

  return (
    <div className="text-xl mx-4">
      {beaches &&
        beaches.map((shopping) => (
          <div
            key={shopping.id}
          ...
          >
            <div className="flex-shrink-0 grid">
              <Image
                src={`/images/${places.indexOf(shopping)}.jpg`}
                width={312}
                height={208}
                alt={shopping.name}
              />
            ...
            <div className="border-r border-b border-l border-gray-400 lg:border-l-0 lg:border-t lg:border-gray-400 bg-white rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
              ...
              <div className="px-1 pb-3">
                <p className="text-gray-700 text-base">
                  {spots[shopping.id].description}
                </p>
              </div>
              <div className="flex items-center px-1 pb-4">
                <div className="text-sm">
                  <p className="text-gray-500">{shopping.address}</p>
                </div>
              </div>
              <div>
                <Link href={`/locations/${places.indexOf(shopping)}`}>
